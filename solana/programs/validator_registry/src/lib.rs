...
pub mod validator_registry {
    ...
    pub fn add_validator(ctx: Context<Manage>, validator: EvmAddr) -> Result<()> {
        require!(validator != [0u8; 20], VrErr::ZeroValidator);
        let r = &mut ctx.accounts.registry;
        match r.validators.binary_search(&validator) {
            Ok(_) => return err!(VrErr::DuplicateValidator),
            Err(pos) => {
                require!(r.validators.len() < MAX_VALIDATORS, VrErr::TooManyValidators);
                r.validators.insert(pos, validator);
            }
        }
        Ok(())
    }

    pub fn remove_validator(ctx: Context<Manage>, validator: EvmAddr) -> Result<()> {
        let r = &mut ctx.accounts.registry;
        let pos = r
            .validators
        ...
        // EVM re-validates the threshold against the new count on removal.
        validate_threshold(r.threshold as usize, r.validators.len())?;
        Ok(())
    }

    pub fn replace_validator(ctx: Context<Manage>, old: EvmAddr, new: EvmAddr) -> Result<()> {
        require!(new != [0u8; 20], VrErr::ZeroValidator);
        let r = &mut ctx.accounts.registry;
        let pos = r
            .validators
            .binary_search(&old)
            .map_err(|_| error!(VrErr::ValidatorNotFound))?;
        require!(
            r.validators.binary_search(&new).is_err(),
            VrErr::DuplicateValidator
        );
        r.validators.remove(pos);
        let ins = r.validators.binary_search(&new).unwrap_err();
        r.validators.insert(ins, new);
        Ok(())
    }

    pub fn set_threshold(ctx: Context<Manage>, new_threshold: u8) -> Result<()> {
        let r = &mut ctx.accounts.registry;
...
impl Registry {
    pub const SIZE: usize = 8 + 32 + 1 + (4 + 20 * MAX_VALIDATORS) + 1;
